...
public class SmartQQClient implements Closeable {
	// 日志
	private static final Logger LOGGER = Logger.getLogger("smart");
	// 发生ngnix 404 时的重试次数
	private static int retryTimesOnFailed = 3;
	// 消息id，这个好像可以随便设置，所以设成全局的
	private volatile static long MESSAGE_ID = 43690001;
	// 客户端id，固定的
	...
	// 线程开关
	private volatile boolean pollStarted = true;
	...
	public SmartQQClient(String tag) {
	...
	public boolean login(long waitTime, final MessageCallback callback) {
		...
		setPollMessage(null);
		sleep(3);
		setPollMessage(callback);
		return true;
	}

	private synchronized void setPollMessage(MessageCallback callback) {
		pollRunnable.callback = callback;
	...
	private void getQRCode() {
		...
		String filePath;
		try {
			filePath = FileUtil.webInfFile("sres/img/", tag + ".png").getCanonicalPath();
		} catch (IOException e) {
			throw new IllegalStateException("二维码保存失败");
		}
		Response<File> response = session.get(ApiURL.GET_QR_CODE.getUrl()).addHeader("User-Agent", ApiURL.USER_AGENT).file(filePath);
	...
	// 登录流程2：校验二维码
	private String verifyQRCode(long waitTime) {
	...
	public void sendMessageToGroup(long groupId, String... msg) {
		for (int i = 0; i < msg.length; i++) {
			...
			r.put("content", JSON.toJSONString(Arrays.asList(msg[i], Arrays.asList("font", Font.DEFAULT_FONT)))); // 注意这里虽然格式是Json，但是实际是String
			r.put("face", 573);
			r.put("clientid", Client_ID);
	...
	private static JSONObject getResponseJson(Response<String> response) {
		...
		if (retCode == null) {
			...
			switch (retCode) {
			case 103: {
				// LOGGER.error("请求失败，Api返回码[103]。查看是否恢复正常");
				break;
			}
			case 100100: {
	...
	public void close() {
		...
		if (pollRunnable.callback instanceof Closeable) {
			try {
				((Closeable) pollRunnable.callback).close();
			} catch (Exception e) {
				LOGGER.error("关闭出错1");
			}
		}
