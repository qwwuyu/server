...
public class MultipartFileSender {
    ...
    public void serveResource() throws IOException {
        ...
        if (StringUtils.isEmpty(fileName)) {
            J2EEUtil.renderInfo(response, "文件未符合", HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
            return;
        }
        ...
        if (ifMatch != null && !HttpUtils.matches(ifMatch, etag)) {
            J2EEUtil.renderInfo(response, "no If-Match", HttpServletResponse.SC_PRECONDITION_FAILED);
            return;
        }
        ...
        if (ifUnmodifiedSince != -1 && ifUnmodifiedSince + 1000 <= lastModified) {
            J2EEUtil.renderInfo(response, "no If-Unmodified-Since", HttpServletResponse.SC_PRECONDITION_FAILED);
            return;
        }
